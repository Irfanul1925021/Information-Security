...
unsigned char hexDecNum[10];


void getBinary(unsigned char arr[10]){
...
// The first round key is the key itself.
   for (i=0 ; i < Nk ; i++) {
      ...
      unsigned char hexDecNum[10] ;
      hexDecNum  = RoundKey[i*4] ;
      getBinary(RoundKey[i*4]) ;
      RoundKey[i*4+1] = Key[i*4+1];
        cout << RoundKey[i*4+1] << "KhankirPola " ;

      RoundKey[i*4+2] = Key[i*4+2];
            cout << RoundKey[i*4+2] << "KhankirPola " ;

      RoundKey[i*4+3] = Key[i*4+3];
            cout << RoundKey[i*4+3] << "KhankirPola " ;
...
void AddRoundKey(int round) {
   ...
   //cout << RoundKey[round * Nb * 4 + i * Nb + j] << " " ;
   //cout << "Bainchod : ";
   //cout << state[j][i] << " ";
      }
      cout << endl;
